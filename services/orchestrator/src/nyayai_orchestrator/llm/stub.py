...
def _extract_first_json_object(raw: str) -> dict[str, Any] | None:
    ...
    The agent prompts dump structured input as JSON. Regex with sorted keys
    is fragile, so we walk the string to find a balanced ``{...}`` block and
    json.loads it.
    """
    start = raw.find("{")
    while start != -1:
...
def _canned_counterfactual(messages: list[ChatMessage]) -> dict[str, Any]:
    """Stub CounterfactualNarrative. The agent re-stamps audit_id + severity
    post-call so the values we emit here only need to satisfy schema."""
    audit_id = _extract_audit_id(messages)
    raw = " ".join(m.content for m in messages)
    payload = _extract_first_json_object(raw) or {}
    rate = float(payload.get("directional_flip_rate") or 0.0)
    severity = (
        "action_required" if rate >= 0.15 else
        "advisory" if rate >= 0.05 else
        "info"
    )
    pct = f"{rate * 100:.1f}%"
    if severity == "info":
        headline = (
            f"Counterfactual stability: only {pct} of decisions change when the "
            "protected attribute is swapped."
        )
    else:
        headline = (
            f"{pct} of sampled decisions flip when the protected attribute is "
            "swapped — direct discrimination signal."
        )
    interpretation = (
        f"The classical flip-test sampled rows from the dataset and re-scored "
        f"each one with the protected attribute swapped to every other value. "
        f"In aggregate, {pct} of decisions changed. A non-zero directional "
        f"flip rate is consistent with direct discrimination — i.e. the model "
        f"itself uses the protected attribute as a feature. If this rate is "
        f"low, observed group disparity is more likely driven by correlated "
        f"features (see Root-Cause)."
    )
    ...
    for ex in examples[:5]:
        if not isinstance(ex, dict):
            continue
        before = ex.get("protected_value_before")
        after = ex.get("protected_value_after")
        db = int(ex.get("decision_before", 0))
        da = int(ex.get("decision_after", 0))
        if not before or not after or db == da:
            continue
        verb = "approved" if da > db else "denied"
        takeaways.append(
            f"Same row, attribute swapped {before}→{after}: decision {verb}."
        )
    return {
        "audit_id": audit_id,
        ...
        "severity": severity,
        "example_takeaways": takeaways[:5],
        "disclaimer": (
            "Counterfactual flip rates are computed by the classical Fairness "
            "Metrics service. The interpretation is generated by Gemini 3 "
            "Flash and is non-binding advisory text."
        ),
    }


def _canned_root_cause(messages: list[ChatMessage]) -> dict[str, Any]:
    """Stub RootCauseNarrative. Agent re-stamps top_drivers numbers after."""
    audit_id = _extract_audit_id(messages)
    raw = " ".join(m.content for m in messages)
    payload = _extract_first_json_object(raw) or {}
    ...
    for r in rankings[:5]:
        ...
        contrib = r.get("contribution_to_disparity")
        if not name or contrib is None:
            continue
        try:
            c = float(contrib)
            ...
            continue
        explanation = (
            f"Feature {name!r} accounts for a {c:+.3f} permutation-importance "
            "shift in the demographic-parity gap. In Indian datasets, common "
            "proxies include pincode (residential segregation), surname "
            "(caste signal), and education_institution (caste/class signal); "
            "review whether this feature falls into one of those categories "
            "before deciding to retain it."
        )
        drivers.append(
            {
                "feature_name": name,
                "contribution_to_disparity": c,
                "plain_explanation": explanation[:400],
            }
        )
    if not drivers:
        drivers = [
            {
                ...
                "contribution_to_disparity": 0.0,
                "plain_explanation": (
                    "No feature contributions were supplied; root-cause analysis "
                    "is unavailable for this audit."
                ),
            }
        ]
    proxies = payload.get("proxy_features") or []
    proxy_warnings = [
        f"{p!r} likely acts as a proxy for the protected attribute in this dataset."
        for p in proxies[:8] if isinstance(p, str)
    ]
    suggested = [
        "Drop or hash the strongest proxy features before retraining.",
        "Add a fairness constraint to the training loop that forbids using proxy features below the district level.",
        "Run intersectional slicing (caste × gender) to confirm the proxy persists across subgroups.",
    ]
    return {
        "audit_id": audit_id,
        "headline": (
            f"Top driver: {drivers[0]['feature_name']!r} (contribution "
            f"{drivers[0]['contribution_to_disparity']:+.3f})."
        )[:300],
        "top_drivers": drivers,
...
class StubGeminiClient(GeminiClient):
    ...
    def generate_structured(
        ...
        *,
        ...
        elif _looks_like_schema(response_schema, "CounterfactualNarrative"):
            parsed = _canned_counterfactual(messages)
        elif _looks_like_schema(response_schema, "RootCauseNarrative"):
            parsed = _canned_root_cause(messages)
